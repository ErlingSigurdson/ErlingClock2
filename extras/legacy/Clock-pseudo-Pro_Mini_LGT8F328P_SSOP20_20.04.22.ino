...
// Note: some LGT8F328P chips fail to consistently run at 32 MHz internal clock. Switch to 16 MHz internal clock in case of any troubles.
...
// dispay_digit()
  ...
  const uint8_t D1_PIN = A1;
  const uint8_t D2_PIN = A0;
  const uint8_t D3_PIN = 13;
  const uint8_t D4_PIN = 12;
...
// manual_settime()
  ...
  const uint8_t SETTIME_HOURS_PIN = 3;              // Doubles as hold-toggle pin.
  const uint8_t SETTIME_MINUTES_PIN = 2;
  ...
  if (settime_hours_button_is_pressed && settime_hours_button_wasnt_pressed) {
    ...
    if (settime_hours_button_is_pressed) {
      ...
      while (holding_toggle_available) {
        settime_hours_button_is_pressed = !digitalRead(SETTIME_HOURS_PIN);
        if (!settime_hours_button_is_pressed) {
          holding_toggle_available = false;        // Escape from while() loop without toggling settime mode.
        }
        if (millis() - previous_millis >= holding_toggle_timing) {
          holding_toggle_available = false;        // Not really necessary, but it feels right to drop this flag to zero before settime sequence begins.
